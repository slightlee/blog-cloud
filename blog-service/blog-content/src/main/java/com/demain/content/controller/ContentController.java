...
//	private final RestTemplate restTemplates;
	...
	public Result<ContentInfoV> getContentInfo(@PathVariable("id") Long id) {
		...
		ResponseEntity<User> responseEntity = restTemplate.getForEntity("http://127.0.0.1:8001/user/getUserInfo/{id}", User.class, id);
//		ResponseEntity<User> responseEntity = restTemplates.getForEntity("http://127.0.0.1:8001/user/getUserInfo/{id}", User.class, id);
